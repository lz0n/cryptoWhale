...
token="METfjsdjfiedspealspcx",
    default_scope=1272879520848023585,
    intents=interactions.Intents.ALL
)
betchannel = 1279800756152700979
coinflipchannel = 1279800438295892160
giveawaychannel = 1279800412903444480
depositchannel = 1281690585974968392
withdrawchannel = 1281690606116274296
coinflipchannellog = 1281690693764513853
jackpotchannel = 1281677824897585296
upgraderchannel  = 1281713681163616369
wallet_id = "ltc-2a5643675645912ff1b01453445b0645453f1"
transfer_key = "M3563IsHwG6453K50zCi24653ZiGkwHanY3535ZN22ZB0z14"
ownerid = 1251558282863317045
ad_ping = "@giveways"
tax = 7
withdraw_tax = 0
...
async def on_ready():
    print('Bot is ready!')
    await bot.change_presence(status=interactions.Status.ONLINE, activity=interactions.Activity(name=f"Quality",type=interactions.ActivityType.COMPETING))
...
async def balance(ctx):
    try:
        ...
        if ctx.channel_id != betchannel:
            invalid = interactions.Embed(color=0x26d394, title="Whoopsie! Wrong Channel!", description=f"Please type this command in the <#{betchannel}> Channel!")
...
async def deposit(ctx):
    try:
        ...
        getbalance(str(ctx.author.id))
        if ctx.channel_id != betchannel:
            stock = interactions.Embed(color=0x26d394, title="Whoopsie! Wrong Channel!", description=f"Please type this command in the <#{betchannel}> Channel!")
            ...
            invoice.add_field(name = "Information", value="You will receive notifications.", inline=False)
            ...
            confirmed = interactions.Embed(color=0x26d394, title="Deposit (STATUS)")
            confirmed.add_field(name = "Information",value=f"Your deposit of ${round(usd,2)} has been processed.", inline=False)
            channel = bot.get_channel(depositchannel)
            await channel.send(embeds=confirmed, content=f'deposited by <@{ctx.author.id}>')
...
async def coinflip(ctx, bet: float):
    try:
        ...
        getbalance(str(ctx.author.id))
        if ctx.channel_id != betchannel:
            stock = interactions.Embed(color=0xFF0000, title="Whoopsie! Wrong Channel!", description=f"Please type this command in the <#{betchannel}> Channel!")
            await ctx.send(embeds=stock, ephemeral=True)
                    ...
                    if oldid != component.ctx.author.id:
                        if getbalance(component.ctx.author.id) < bet:
                            ...
                            if rng > 0.5:
                                ...
                                add(str(1279444769944113182), cut(pot, tax))
                            else:
                                coinflip = interactions.Embed(color=0x008000, title="Coinflip")
    ...
    try:
        ...
        if ctx.channel_id != betchannel:
            stock = interactions.Embed(
                ...
                description=f"Please type this command in the <#{betchannel}> Channel!",
            )
            await ctx.send(embeds=stock, ephemeral=True)
